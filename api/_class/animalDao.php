...
class AnimalDao {
    ...
    public function selectByIdForm($animal) {
        ...
        try {
            $mysql = new GDbMysql();
            $mysql->execute(
                "SELECT ani_int_codigo,pro_int_codigo,rac_int_codigo,ani_var_nome,ani_cha_vivo,ani_dec_peso, pro_var_nome FROM vw_animal WHERE ani_int_codigo = ? ", array("i", $animal->getAni_int_codigo()), true, MYSQL_ASSOC);
            if ($mysql->fetch()) {
                $ret = $mysql->res;
