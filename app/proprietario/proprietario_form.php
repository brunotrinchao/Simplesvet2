...
$htmlForm .= $form->addInput('text', 'pro_var_telefone', 'Telefone');
...
<script>
    $(function() {
        ...
        $('#form').submit(function() {
            var usu_int_codigo = $('#pro_int_codigo').val();
            $('#p__selecionado').val();
            if ($('#form').gValidate()) {
                var method = ($('#acao').val() == 'ins') ? 'POST' : 'PUT';
                var endpoint = ($('#acao').val() == 'ins') ? URL_API + 'proprietarios' : URL_API + 'proprietarios/' + usu_int_codigo;
        ...
        $('#f__btn_excluir').click(function() {
            ...
            $.gDisplay.showYN("Quer realmente deletar o item selecionado?", function() {
                $.gAjax.exec('DELETE', URL_API + 'proprietario/' + usu_int_codigo, false, false, function(json) {
                    if (json.status) {
                        showList(true);
                    }
                });
