...
$rac_var_nome = $_POST['p__rac_var_nome'];

if (!empty($rac_var_nome)) {
    $filter->addFilter('AND', 'rac_var_nome', 'LIKE', 's', '%' . str_replace(' ', '%', $rac_var_nome) . '%');
}
...
try {
    if ($type == 'C') {
        $query = "SELECT count(1) FROM vw_raca " . $filter->getWhere();
        ...
        $filter->setOrder(array('rac_var_nome' => 'ASC'));
        $filter->setLimit($start, $rp);

        $query = "SELECT rac_int_codigo, rac_var_nome FROM vw_raca " . $filter->getWhere();
        $param = $filter->getParam();
        ...
        if ($mysql->numRows() > 0) {
            ...
            $html .= '<tbody>';
            while ($mysql->fetch()) {
                $class = ($_POST['p__selecionado'] == $mysql->res['rac_int_codigo']) ? 'success' : '';
                $html .= '<tr id="' . $mysql->res['rac_int_codigo'] . '" class="linhaRegistro ' . $class . '">';
